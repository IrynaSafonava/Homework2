...
public class CalculatorTest {
   ...
   private Calculator testSubject = new Calculator();
    ...
    public void testSummarize(double x, double y, double expectedResult) {
        assertEquals(expectedResult, testSubject.toSummarize(x, y),
                String.format("Summing is failed for %s and %s", x, y));
    }
    ...
    public void testSubtract(double x, double y, double expectedResult) {
        assertEquals(expectedResult, testSubject.toSubtract(x, y),
                String.format("Subtraction is failed for %s and %s", x, y) );
    }
    ...
    public void testMultiply(double x, double y, double expectedResult) {
        assertEquals(expectedResult, testSubject.toMultiply(x, y),
                String.format("Multiplication is failed for %s and %s", x, y));
    }

    @ParameterizedTest
    @CsvSource(value = {"-5, 0", "0, -5", "53.89, 0", "-2147483647, 0"})
    public void testMultiplyByZero(double x, double y) {
        assertEquals(0, testSubject.toMultiply(x, y),
                String.format("Multiplication by zero is failed for %s and %s", x, y));
    }
    ...
    public void testDivide(double x, double y, double expectedResult) {
        assertEquals(expectedResult, testSubject.toDivide(x, y),
                String.format("Division is failed for %s and %s", x, y));
    }
